    ...
    try:
        for _, row in data.iterrows():
            ...
            if vertex_id in graph.vs["name"]:
                ...
                graph.vs[vertex_index]["Minimum_Care_Time"] = row[
                    "tempo_cuidados_minimos"
                ]
...
def problem_data_dict_by_each_file(
    initial_data_file: str, points_data_file: str, edges_data_file: str
) -> Dict[str, Any]:
    """
    Loads data and builds the full igraph Graph with attributes.
    Returns a dictionary with the graph, points data, ruas data, and initial data.
...
class TestStart(unittest.TestCase):
    ...
    def test_initial_data_df(self):
        df = initial_data_df(
            "/home/pedrom/Documentos/SciTech/SciTech/Docs/Dataset de Test/datasets/hard/10/dados_iniciais.csv"
        )
        self.assertIsNotNone(df)
        self.assertEqual(df.iloc[0]["ponto_inicial"], 0)

    def test_problem_data_df(self):
        data = problem_data_dict_by_folder(
            "/home/pedrom/Documentos/SciTech/SciTech/Docs/Dataset de Test/datasets/hard/10"
        )
    ...
    def test_show_hard_10_data(self):
        ...
        print(
            f"Graph: {data['graph'].vcount()} vertices, {data['graph'].ecount()} edges"
        )
        print("Points Data:")
