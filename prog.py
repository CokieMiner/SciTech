...
from Start import pd_to_igraph, add_points_data_to_graph  # Usa o teu código original
...
def precompute_all_pairs_shortest_paths(graph: igraph.Graph) -> Tuple[Dict[Tuple[int, int], float], Dict[Tuple[int, int], List[int]]]:
    ...
    for v in range(len(graph.vs)):
        ...
        for u, path in enumerate(spaths):
            if path:
                dist = sum(graph.es[graph.get_eid(path[i], path[i+1])]['weight'] for i in range(len(path)-1))
            else:
                dist = float('inf')
            distances[(v, u)] = dist
            paths[(v, u)] = path
...
def select_next_patient_optimized(current_node: int, patients: pd.DataFrame,
                                  time_left: float, hospitals: List[int],
                                  distances: Dict[Tuple[int,int], float],
                                  paths: Dict[Tuple[int,int], List[int]]) -> Optional[Tuple[int, List[int], List[int], float]]:
    """
    ...
    for _, patient in patients.iterrows():
        ...
        total_time_needed = dist_to_patient + patient['tempo_cuidados_minimos'] + min_return_dist
        if total_time_needed <= time_left:
            candidates.append((patient_id, paths[(current_node, patient_id)], best_path_to_hospital, total_time_needed, patient['prioridade']))
...
def ambulance_routing_optimized(graph: igraph.Graph, points_data: pd.DataFrame,
                                initial_point: int, total_time: float) -> List[Dict[str, Any]]:
    """
    Simula a operação da ambulância, retornando uma lista de trajetos realizados.
    """
    hospitals = points_data[points_data['tipo'].str.lower() == 'hospital']['id'].tolist()
    ...
    while time_left > 0 and not patients.empty:
        next_task = select_next_patient_optimized(current_node, patients, time_left, hospitals, distances, paths)
        if not next_task:
            break
        ...
        route_log.append({
            ...
            "priority": points_data.loc[points_data['id'] == patient_id, 'prioridade'].values[0]
        })
...
if __name__ == "__main__":
    log = run_from_csv_optimized("/home/maria-paz/Secretária/hachaton/datasets/easy/1")
    for step in log:
        print(step)
